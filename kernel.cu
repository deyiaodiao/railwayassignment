...
void _showPath(int start, int end, const vector<Piii>& path, const double* D, const int N);
bool _getPath(int curEdge, int nxtEdge, vector<Piii>& path, const double* D, const int* Dpath, const int N);
void _get_full_paths(const double* D, const int* Dpath, const int N);

//CUDA GPU kernel/functions forward declaration
__global__ void _Wake_GPU(int reps);
__global__ void _GPU_Floyd_kernel(int k, double* G, int* P, int N);
void _GPU_Floyd(double* H_G, int* H_Gpath, const int N);

//other optional utility functions
int _read_from_file(int* G, const int N);
void _generateRandomGraph(double* G, int N, int range, int density);
...
int main() {
	...
	if (DO_TEST_RANDOM) {//will use the #define(s) to init a random adjacency Matrix of RANDOM_GSIZE size
		const int NumBytes = RANDOM_GSIZE * RANDOM_GSIZE * sizeof(double);
		//host allocations to create Adjancency matrix and result matrices with path matrices
		double* OrigGraph = (double*)malloc(NumBytes);//will be original Adjancency matrix, will NOT be changed
		double* H_G = (double*)malloc(NumBytes);
		int* H_Gpath = (int*)malloc(NumBytes);
		double* D_G = (double*)malloc(NumBytes);
...
__global__ void _GPU_Floyd_kernel(int k, double* G, int* P, int N) {//G will be the adjacency matrix, P will be path matrix
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	if (col >= N)return;
	int idx = N * blockIdx.y + col;

	__shared__ double best;
	if (threadIdx.x == 0)
		best = G[N * blockIdx.y + k];
	__syncthreads();
	if (best == INF)return;
	double tmp_b = G[k * N + col];
	if (tmp_b == INF)return;
	double cur = best + tmp_b;
	if (cur < G[idx]) {
		G[idx] = cur;
		P[idx] = k;
	}
...
void _generateRandomGraph(double* G, int N, int range, int density) {//density will be between 0 and 100, indication the % of number of directed edges in graph
